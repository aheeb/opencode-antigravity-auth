# Task: 05-add-tests-for-providermetadata-conversion
...
- 04-add-signature-recovery-fallback: Added SENTINEL_SIGNATURE fallback ("skip_thought_signature_validator") in ensureThoughtSignature(). When cache lookup fails to recover a signature, the sentinel is used instead of leaving the thinking block unsigned. This prevents Claude API rejection and allows graceful redaction.
